        ...
        if gaussians.binding != None:
            ...
            if opt.lambda_scale != 0:
                if opt.metric_scale:
                    losses["scale"] = (
                        F.relu(
                            gaussians.get_scaling[visibility_filter]
                            - opt.threshold_scale
                        )
                        .norm(dim=1)
                        .mean()
                    ...
                    losses["scale"] = (
                        F.relu(
                            torch.exp(gaussians._scaling[visibility_filter])
                            - opt.threshold_scale
                        )
                        .norm(dim=1)
                        .mean()
            ...
            if opt.lambda_dynamic_offset != 0:
                losses["dy_off"] = (
                    gaussians.compute_dynamic_offset_loss() * opt.lambda_dynamic_offset
                )
            ...
            if opt.lambda_dynamic_offset_std != 0:
                ...
                if ti < gaussians.num_timesteps - 1:
                    t_indices.append(ti + 1)
                losses["dynamic_offset_std"] = (
                    gaussians.flame_param["dynamic_offset"].std(dim=0).mean()
                    * opt.lambda_dynamic_offset_std
                )

            if opt.lambda_laplacian != 0:
                losses["lap"] = (
                    gaussians.compute_laplacian_loss() * opt.lambda_laplacian
                )
